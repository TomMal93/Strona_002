...
test('site content exposes required SEO, Hero, About and Services copy fields', () => {
  ...
  for (const item of siteContent.services.items) {
    ...
    assert.equal(typeof item.video.src, 'string')
    assert.equal(typeof item.video.poster, 'string')
    ...
    assert.ok(item.video.src.length > 0)
    assert.ok(item.video.poster.length > 0)
  }
