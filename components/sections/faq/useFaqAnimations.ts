...
export function useFaqAnimations(refs: FaqAnimationRefs): void {
  useLayoutEffect(() => {
    ...
    const setInitialStyles = () => {
      const { titleRef, hudBarRef, itemsContainerRef } = refs
      ...
      const cornerMarks = refs.sectionRef.current
        ...
        : []
      const faqItems = itemsContainerRef.current
        ? Array.from(itemsContainerRef.current.querySelectorAll<HTMLElement>('[data-faq-item]'))
        : []
      const filmMarkers = refs.sectionRef.current
        ? Array.from(refs.sectionRef.current.querySelectorAll<HTMLElement>('[data-film-marker]'))
        : []
    ...
    const initAnimations = async () => {
      ...
      const { sectionRef, titleRef, subtitleRef, hudBarRef, itemsContainerRef } = refs
      ...
      const ctx = gsap.context(() => {
        ...
        const faqItems = itemsContainerRef.current
          ? Array.from(itemsContainerRef.current.querySelectorAll('[data-faq-item]'))
          : []
        const filmMarkers = Array.from(
          sectionRef.current?.querySelectorAll('[data-film-marker]') ?? [],
        )
        ...
        if (prefersReducedMotion) {
          ...
          if (faqItems.length) gsap.set(faqItems, { autoAlpha: 1, y: 0 })
          if (filmMarkers.length) gsap.set(filmMarkers, { autoAlpha: 1, y: 0 })
          return
        }
        ...
        if (faqItems.length) gsap.set(faqItems, { autoAlpha: 0, y: 20 })
        if (filmMarkers.length) gsap.set(filmMarkers, { autoAlpha: 0, y: 10 })
